...
Now this is close(!), what we want is $$x_1+x_2+x_3 \geq 3(x_1x_2x_3)^{1/3}$$. But we got this $$x_1+x_2+x_3 \geq (x_1 x_2 x_3)^{1/2} \left(x_1^{-1/2} + x_2^{-1/2} + x_3^{-1/2}\right)$$ instead. What can we do? note the sum $$x_1^{-1/2} + x_2^{-1/2} + x_3^{-1/2}$$. What we just did with $$x_1+x_2+x_3$$, we can do it withe the $$x_1^{-1/2} + x_2^{-1/2} + x_3^{-1/2}$$ too! i.e. we can do the following

$$
\begin{align*}
x_1^{-1/2} + x_2^{-1/2} + x_3^{-1/2} &\geq (x_1^{-1/2}x_2^{-1/2}x_3^{-1/2})^{1/2} \left( \left(x_1^{-1/2}\right)^{-1/2} + \left(x_2^{-1/2}\right)^{-1/2} + \left(x_3^{-1/2}\right)^{-1/2} \right) \\
